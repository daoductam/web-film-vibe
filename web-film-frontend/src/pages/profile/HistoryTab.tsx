        ...
        <div>
            ...
            <div className="space-y-4">
                {history.map((item, idx) => (
                    <Link key={item.movieSlug + idx} to={`/movie/${encodeURIComponent(item.movieSlug)}?episode=${encodeURIComponent(item.lastEpisodeSlug || '')}`} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors border border-white/5 group">
                        <div className="w-32 aspect-video bg-black rounded-lg overflow-hidden shrink-0 relative">
                             ...
                             <div className="absolute bottom-0 left-0 h-1 bg-neon" style={{ width: `${Math.min(100, Math.max(0, (item.progressMs / (item.durationMs || 1)) * 100))}%` }}></div>
                        </div>
