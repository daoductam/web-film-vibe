...
export const PopularPage = () => {
    ...
    // Sort State (Default to Views for "Popular" feel, or Updated for "New")
    // Let's default to View Count since it's "Popular" page
    ...
    const { data: moviesData, isLoading } = useQuery({
        queryKey: ['popular-movies', filters, sortBy, page],
        queryFn: () => movieService.filterMovies({
            // No specific type forced, shows ALL movies (Single + Series)
        ...
        <div className="bg-obsidian text-text-primary font-sans overflow-x-hidden antialiased selection:bg-neon selection:text-obsidian min-h-screen flex flex-col">
            ...
            <main className="flex-1">
                        ...
                        {featuredMovie ? (
                            <div className="max-w-3xl space-y-6 animate-fade-in-up">
                                <div className="flex flex-wrap gap-3 items-center">
                                    ...
                                    <span className="text-gray-300 text-sm font-medium">{featuredMovie.year} • {featuredMovie.countries?.[0]?.name}</span>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                            ...
                            <div className="flex items-center gap-2 text-text-secondary text-sm whitespace-nowrap">
                                ...
                                <button 
                                    ...
                                    className={`transition-colors font-bold ${sortBy === 'updatedAt,desc' ? 'text-neon underline' : 'hover:text-white'}`}
                                >
                                    Mới nhất
                                </button>
                                <span>|</span>
                                <button 
                                    ...
                                    className={`transition-colors font-bold ${sortBy === 'viewCount,desc' ? 'text-neon underline' : 'hover:text-white'}`}
                                >
                                    Xem nhiều nhất
                                </button>
                            </div>
                ...
                <section className="max-w-[1600px] mx-auto px-6 py-12">
                    <h2 className="text-2xl font-serif font-bold text-white mb-8 border-l-4 border-neon pl-4">
                        {sortBy === 'viewCount,desc' ? 'Top Xem Nhiều' : 'Mới Cập Nhật'}
                    </h2>
                            ...
                            {moviesData?.content?.map((movie: any) => (
                                <Link key={movie.id} to={`/movie/${movie.slug}`} className="group relative cursor-pointer interactive-card rounded-2xl glass-card border border-white/5 bg-surface overflow-hidden block">
                                    <div className="aspect-[2/3] w-full relative">
                                        ...
                                        {movie.type === 'SERIES' && (
                                             <div className="absolute bottom-2 left-2 z-20 px-2 py-0.5 bg-black/60 backdrop-blur rounded text-white text-[10px] font-bold border border-white/10">
