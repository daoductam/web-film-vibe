...
- **Codebase Memory (MCP):** Sử dụng `codebase-memory-mcp` ở giai đoạn **Understand / Research / Investigate** của mọi workflow để nhanh chóng truy vết mã nguồn thay vì quét file thủ công (xem chi tiết quy tắc ưu tiên tại cấu hình hệ thống).
